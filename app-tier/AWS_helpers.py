...
class HelperAPI:
    ...
    def push_to_inputbucket(self, message):
    ...
    def remove_from_in_queue(self, receipt, file_name):
        return self.delete_from_queue(self.input_queue_url, receipt, file_name)

    def delete_from_output_queue(self, receipt, file_name):
        return self.delete_from_queue(self.output_queue_url, receipt, file_name)

    def delete_from_queue(self, url, receipt, file_name):
        if url == self.input_queue_url:
            print("Deleting item from Input SQS: " + file_name)
        else:
            print("Deleting item from Output SQS: " + file_name)
        try:
            response = self.sqs.delete_message(QueueUrl = url, ReceiptHandle = receipt)
    ...
    def queue_attributes_input(self, attribute_name):
